...
if ($status == true) {
    ...
    $txResults = null;
    $eth->sendRawTransaction($signedTxs, function ($err, $txHash) use (&$msg, &$trstatus, $eth) {
        if ($err) {
            // Handle error
            $msg = $err;
            $trstatus = false;
        } else {
            // $txHash is defined as the callback parameter and should be available here
            // Now, to check the transaction status, we need to call getTransactionReceipt or a similar method
            // Assuming getTransactionReceipt is available and correctly implemented in your $eth object
            $eth->getTransactionReceipt($txHash, function ($err, $receipt) use (&$msg, &$trstatus, $txHash) { // Note the use of $txHash here
                if ($err) {
                    // Handle error in fetching the receipt
                    $msg = $err;
                    $trstatus = false;
                } else {
