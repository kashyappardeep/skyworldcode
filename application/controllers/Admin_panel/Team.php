...
class Team extends CI_Controller{
    ...
    public function team_tree(){
        ...
        if(isset($_REQUEST['node'])){
            ...
            $data['node_id']=1;  //$this->session->userdata('user_id');
        }
          if(isset($_POST['search'])){
                
            $u_code=1;      //$this->session->userdata('user_id');
 ...
 /*    public function team_matrix_generation(){
    ...
    */
    ...
    public function team_member(){ 
        ...
        $searchdata['from_table']='users';
        $tx_username=$_REQUEST['username'];
       // print_r($tx_username);
       $tx_u_code=$this->profile->id_by_username($tx_username);
        
         if(isset($_POST['block_btn'])){
         if(isset($_POST['block_ids'])){
              //  print_r($_POST['block_ids']);
              //die();
                $block_id=$_POST['block_ids'];
                $set['block_status']=1;
                $this->db->where_in('id',$block_id);
                $this->db->update('users',$set);
                
               
                $this->session->set_flashdata("success", " Block User.");
                redirect($this->admin_url.'/team/team_member');
            }else{
                $this->session->set_flashdata("error", "Invalid Request. Please Enter username");
                redirect($this->admin_url.'/team/team_member'); 
            }
        ...
        if(isset($_POST['unblock_btn'])){
         if(isset($_POST['unblock_ids'])){
                //print_r($_POST['block_ids']);
               // die();
                $unblock_id=$_POST['unblock_ids'];
                $set['block_status']=0;
                $this->db->where_in('id',$unblock_id);
                $this->db->update('users',$set);
                
               
                $this->session->set_flashdata("success", "UnBlock User.");
                redirect($this->admin_url.'/team/team_member');
            }else{
                $this->session->set_flashdata("error", "Invalid Request. Please Enter username ");
                redirect($this->admin_url.'/team/team_member'); 
        ...
        $data['user_id']=$tx_u_code;
        $this->show->admin_panel('users_team_block',$data);
    } 
