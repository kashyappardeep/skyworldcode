    ...
    public function index(){
       ...
       $searchdata['order_by']='id desc'; 
        ...
        if(isset($_REQUEST['select_wallet']) && $_REQUEST['select_wallet']!=''){
            $wallet_select=$_REQUEST['select_wallet'];
            if($wallet_select){
        ...
        $data = $this->paging->search_response($searchdata,$this->limit,$this->admin_url.'/transactions'); 

         $ttl=$this->conn->runQuery('sum(amount)as total,sum(tx_charge)as charge',  'transaction',"status=1 $where");
        ...
        if(isset($_REQUEST['export_to_excel'])){
           ...
           $get_data=$this->conn->runQuery('u_code,tx_u_code,debit_credit,tx_charge,remark,tx_type,source,amount,added_on','transaction',"1=1");  
