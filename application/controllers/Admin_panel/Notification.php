    ...
    public function send_message(){
        if(isset($_POST['send_btn'])){
            ...
            if($this->form_validation->run() != false){
                $arr=array();
                $get_id=$this->profile->id_by_username($_POST['tx_username']);
                $tx_profile=$this->profile->profile_info($get_id);
                $mobile=$tx_profile->mobile;
                
                $message=$_POST['message'];
                $this->message->sms($mobile,$message);
                
                $this->session->set_flashdata('success',"Notification added Successfully.");
                $admin_path=$this->conn->company_info('admin_path');
                redirect(base_url($admin_path.'/notification/send-message'));
            }else{
                $this->session->set_flashdata('error',"Something Wrong.");
            }
        }
        $this->show->admin_panel('notifications/send_message');
    }
