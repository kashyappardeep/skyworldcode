    ...
    /*    $u_code=$this->session->userdata("admin_id");
        ...
        if($check_ex){
            ...
            if(!empty($cart_contant)){
                $cart_qtys=json_decode(json_encode(array_column($cart_contant,'qty','id')),true);
            }
            ...
            if(!$required_fielsa){
                $product_stock=$this->product->admin_stock($pro_id);
   ...
   function placeOrder($ordData,$u_code){       
        ...
        if($insertOrder){
            ...
            foreach($cartItems as $item){
                ...
                $ordItemData[$i]['sub_mrp']     = $item["mrp"];
    ...
    public function order_product_view_old(){
        if(isset($_REQUEST['order_id'])){
            $this->session->set_userdata('admin_order_item_id',$_REQUEST['order_id']);
        }
        $product_id=$this->session->userdata('admin_order_item_id');        
        ...
        if(isset($_POST['approve_btn'])){
            ...
            if($this->form_validation->run() != False){
                ...
                $set['remark']=$_POST['remark'];  
                 $set['updated_on']=$today;   
                $this->db->where('id',$product_id);
                $this->db->update('franchise_orders',$set);               
               
                redirect(base_url($this->conn->company_info('admin_path').'/franchise/franchise_delevered_items'));
            }
        }
        $data['product_id']=$product_id;
        $this->show->admin_panel('product/item_view',$data);
        
    }   
    
    
    
  public function franchise_all_orders(){
        ...
        $data = $this->paging->search_response($searchdata,$this->limit,$this->admin_url.'/franchise/purchase');
        $data['search_string']='admin_franchise_purchase_search';
        $this->show->admin_panel('franchise_all_orders',$data);
        
    }
    
     public function franchise_delevered_items(){
        ...
        $data = $this->paging->search_response($searchdata,$this->limit,$this->admin_url.'/franchise/purchase');
        $data['search_string']='admin_franchise_purchase_search';
        $this->show->admin_panel('product/item_delivered',$data);
