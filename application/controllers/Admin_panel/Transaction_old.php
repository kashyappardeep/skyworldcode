<?php
class Transaction_old extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if (isset($_REQUEST['income'])) {
            $this->session->set_userdata('show_income',trim($_REQUEST['income']));
        }
        ...
        $where = '';
        $searchdata['from_table'] = 'transaction_bkp';
        $searchdata['order_by'] = 'id desc';
        if (isset($_REQUEST['name']) && $_REQUEST['name'] != '') {
            $spo = $this->profile->column_like(trim($_REQUEST['name']), 'name');


            if ($spo) {
                $conditions['u_code'] = $spo;
                $where .= " and u_code  = $spo[0]";
            }
        }
        if (isset($_REQUEST['username']) && $_REQUEST['username'] != '') {


            $spo = $this->profile->column_like(trim($_REQUEST['username']), 'username');

            if ($spo) {

                $conditions['u_code'] = $spo[0];
                $where .= " and u_code = $spo[0]";
            }
        }
        if (isset($_REQUEST['start_date']) && isset($_REQUEST['end_date']) && $_REQUEST['start_date'] != '' && $_REQUEST['end_date'] != '') {
            $start_date = date('Y-m-d 00:00:00', strtotime(trim($_REQUEST['start_date'])));
            $end_date = date('Y-m-d 23:59:00', strtotime(trim($_REQUEST['end_date'])));
            $wheres = "(date BETWEEN '$start_date' and '$end_date')";
            $where = "and (date BETWEEN '$start_date' and '$end_date')";
            $searchdata['where'] = $wheres;
        }
        if (isset($_REQUEST['source']) && $_REQUEST['source'] != '') {
            $source = trim($_REQUEST['source']);
            if ($source) {
                $conditions['tx_type'] = $source;
                $where .= " and tx_type = $source";
            }
        }
        if (isset($_REQUEST['debit_credit']) && $_REQUEST['debit_credit'] != '') {
            $debit_credit = trim($_REQUEST['debit_credit']);
            if ($debit_credit) {
                $conditions['debit_credit'] = $debit_credit;
                $where .= " and debit_credit  = '$debit_credit'";
            }
        }


        if (isset($_REQUEST['select_wallet']) && $_REQUEST['select_wallet'] != '') {
            $wallet_select = trim($_REQUEST['select_wallet']);
            if ($wallet_select) {
                $conditions['wallet_type'] = $wallet_select;
                $where .= " and wallet_type  = '$wallet_select'";
            }
        }
        ...
        $data = $this->paging->search_response($searchdata, $this->limit, $this->admin_url . '/transaction_old');

        $ttl = $this->conn->runQuery('sum(amount)as total,sum(tx_charge)as charge',  'transaction_bkp', "status=1 $where");

        $data['ttl_amnts'] =   $ttl;
        $this->show->admin_panel('transactions_old', $data);

        if (isset($_REQUEST['export_to_excel'])) {

            $get_data = $this->conn->runQuery('u_code,tx_u_code,debit_credit,tx_charge,remark,tx_type,source,amount,added_on', 'transaction_bkp', "1=1");

            if ($get_data) {

                for ($f = 0; $f < count($get_data); $f++) {
                    $tx_profile = $this->profile->profile_info($get_data[$f]->u_code);
                    $tx_profile_from = $this->profile->profile_info($get_data[$f]->tx_u_code);
                    // $bank_details=json_decode($get_data[$f]->bank_details);
                    $dta['Tx User'] = $tx_profile->username . '( ' . $tx_profile->name . ')';
                    $dta['From User'] = $tx_profile_from->username . '( ' . $tx_profile_from->name . ')';
                    $dta['Debit/Credit'] = $get_data[$f]->debit_credit;
                    $dta['Amount'] = $get_data[$f]->amount;
                    $dta['Tx charge'] = $get_data[$f]->tx_charge;
                    $dta['Txn Type'] = $get_data[$f]->tx_type;
    ...
    */
    ...
    public function pending_payouts()
    {
        ...
        $data = $this->paging->search_response($searchdata, $this->limit, $this->admin_url . '/transaction_bkp/pending_payouts');
    ...
    public function payouts()
    {
        ...
        $data = $this->paging->search_response($searchdata, $this->limit, $this->admin_url . '/transaction_bkp/payouts');
    ...
    public function view()
    {
        ...
        if (isset($_POST['approve_btn'])) {
            ...
            redirect(base_url($this->conn->company_info('admin_path') . '/transaction_bkp/pending_payouts'));
        }
    ...
    {
        $chk_exists = $this->conn->runQuery('id', 'transaction_bkp', "status=0 and id='$wd_id'");
        if ($chk_exists) {
            $set['status'] = 1;
            $set['reason'] = $_POST['reason'];
            $this->db->where('id', $wd_id);
