...
class Product extends CI_Controller{
    ...
    public function index(){
      ...
      public function products(){ 
            ...
            $data['base_url']=$this->admin_url.'/product/products';
     ...
     public function stock(){ 
            ...
            $data['limit']=25;
            $data['from_table']='products';
            $data['base_url']=$this->admin_url.'/product/stock';
            $res=$this->paging->searching_data($data);
            $data['table_data']=$res['table_data'];
            $data['total_rows']=$res['total_rows'];
            $this->show->admin_panel('product/products_stock',$data);  
      }
      ...
      public function product_stock(){ 
            ...
            $data['limit']=25;
            $data['from_table']='products';
            $data['base_url']=$this->admin_url.'/product/product_stock';
            $res=$this->paging->searching_data($data);
            $data['table_data']=$res['table_data'];
            $data['total_rows']=$res['total_rows'];
            $this->show->admin_panel('product/products_stock_franchise_user',$data);  
      }
      public function categories(){  
            $data['search_string']='admin_category_search';
            $data['limit']=25;
            $data['from_table']='categories';
            $data['base_url']=$this->admin_url.'product/categories';
            $res=$this->paging->searching_data($data);
            $data['table_data']=$res['table_data'];
                ...
                $get_insert_id=$this->conn->get_insert_id('products',$insert);
                    ...
                    */
                    ...
                    //die();
                     ...
                     redirect($this->admin_url.'/product/add_product','refresh');
              }else{
           ...
           // print_r($all_tabs);
            ...
            if(isset($_POST['edit_btn'])){
             ...
             /* $upload_product=$this->upload_file->upload_product('file');
              ...
              $upload_product=$this->upload_file->upload_product('file');
              $upload_product1=$this->upload_file->upload_product('morefile');
              $upload_product2=$this->upload_file->upload_product('galaryfile');
              
              if($this->form_validation->run()){
                  
                  
                $morefile=$count = $this->upload_file->upload_multiple('galaryfile');
               
                $json=json_decode($morefile);
                $insert=array();
                if(!empty($json)){
                    $insert['galaryfile']=$morefile;
                }
                
                 if($upload_product1['upload_error']==false){
                    $insert['more_imgs']=base_url().'images/products/'.$upload_product['file_name'];
                }
               ...
               $insert['name']=htmlspecialchars($_POST['product_name']);
                $insert['mrp']=htmlspecialchars($_POST['mrp']);
               $insert['dp']=htmlspecialchars($_POST['dp']);
               $insert['other_dese']=htmlspecialchars($_POST['product_description']);
                $insert['product_bv']=htmlspecialchars($_POST['product_bv']);
                $insert['pv']=htmlspecialchars($_POST['pv']);
                   ...
                   /*if($get_insert_id){
                    ...
                    if(isset($_POST['qty'])){
                      if(is_array($_POST['qty'])){
                        ...
                        $this->db->where('post_id',$product_id);
                        $get_insert_id=$this->db->update('product_skus',$update);
                       }else{
                        $update['qty']=$_POST['qty'];
                        $this->db->where('post_id',$product_id);
                        $get_insert_id=$this->db->update('product_skus',$update);
