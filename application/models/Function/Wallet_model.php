...
class Wallet_model extends CI_Model{
    ...
    ///////////////////////////////////////////////////////////// me
     ...
     public function test_closing(){
        $source="daily_trade";
        $demo=$this->check_status($source);
        if($demo==1){
            echo "code run";
        }else{
            echo"code not run";
  ...
  public function check_status($slug){
        $status=$this->conn->runQuery('closing_status','wallet_types'," slug='$slug' and closing_status ='1' ")[0] ;
        return (!empty($status)?$status->closing_status:0);
   }
