...
$('.check_username_exist').change(function (e) { 
      ...
      $('.check_sponsor_exist').change(function (e) { 
        ...
        $.ajax({
          type: "post",
          url: "https://test.eracom.in/demo_new/admin/invest/check_sponsor_exist",
          ...
          success: function (response) {            
            ...
            if(res.error==true){
              ...
              $('#'+res_area).html(res.msg).css('color','black');              
            }
          }
        });
    });
    ...
    $('.send_otp2').click(function (e) { 
    $(this).html('<i class="fa fa-refresh fa-spin"></i>'); 
    var res_area = $(this).attr('data-response_area');
    
    $.ajax({
            type: "post",
            url: "<?= $admin_path.'settings/send_otp';?>",
            data: {gen_otp:1},          
            success: function (response) {  
            // alert(response);
   ...
   $('.send_otp_edit').click(function (e) { 
    ...
    $.ajax({
            type: "post",
            url: "<?= $admin_path.'settings/send_otp_edit';?>",
            data: {gen_otp:1},          
            success: function (response) {  
            // alert(response);
              $(this).html('Resend OTP'); 
...
<script src="<?php echo $panel_url;?>assets/plugins/jquery-multi-select/jquery.multi-select.js"></script>
  ...
  <?php
 $super_admin_directory=$this->conn->company_info('admin_directory');
   $this->load->view($super_admin_directory.'/pages/settings/scripts');

  ?>
     <?php
        $this->load->view($admin_directory.'/pages/payment_section/scripts');
    ?>
    <script>
        $(document).ready(function() {
