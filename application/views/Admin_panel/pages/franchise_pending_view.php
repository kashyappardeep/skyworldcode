                ...
                <div class="table-responsive">
                  <table class="table table-condensed ">
                    ...
                    <?php  
                      ...
                      if(!empty($table_data)){ 
                        ...
                        // print_r($t_data);
                         ...
                         $name=$this->conn->runQuery('name','products',"id='$product_ids'");
                         ...
                         $total_bv=$this->conn->runQuery('SUM(product_bv*quantity) as ttl_bv','franchise_order_items',"f_code='$order_id' and product_id='$product_ids'")[0]->ttl_bv;
                         $total_mrp=$this->conn->runQuery('SUM(sub_mrp) as ttl_mrp','franchise_order_items',"f_code='$order_id' and product_id='$product_ids'")[0]->ttl_mrp;
                         ...
                         $total_bvs=$this->conn->runQuery('SUM(product_bv*quantity) as tttl_bv','order_items',"franchise_id='$order_id' and product_id='$product_ids'")[0]->tttl_bv;
                       
                        
                         $total_mrps=$this->conn->runQuery('SUM(sub_mrp) as ttls_mrp','order_items',"franchise_id='$order_id' and product_id='$product_ids'")[0]->ttls_mrp;
                              ...
                              $tota_mrp= $total_mrp!='' ? ($total_mrps!='' ? $total_mrp-$total_mrps:$total_mrp):0;
                          ?>
                            <tr>
                              <td><?= $sr+1;?></td>
                              <td><?= $name[0]->name;?></td>
                              <td><?php echo $ttl_qty1[]=$qty_pd;?></td>
                              <td><?= $ttl_mrp[]=$tota_mrp;?></td>
                    ...
                    <tr>
                      ...
                      <td><b> Total Mrp:<?= array_sum($ttl_mrp);?></b></td>
