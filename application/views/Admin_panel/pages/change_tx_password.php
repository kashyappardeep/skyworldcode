...
if($_SESSION['forgot_otp_verified']!==true){
 ...
 if(isset($_POST['change_tx_btn'])){
    
    $this->form_validation->set_rules('tx_password', 'Tx Password', 'trim|required|min_length[6]');
    $this->form_validation->set_rules('confirm_password', 'Password Confirmation', 'trim|required|min_length[6]|matches[tx_password]');
    $update['tx_password'] = md5($_POST['tx_password']);
    if($this->form_validation->run() != False){
        ...
        $this->session->set_flashdata('success',"Tx Password successfully Changed.");
        redirect(base_url('admin/forgot'),"refresh");
		...
		<div class="card-body">
		 <div class="card-content p-2">
		  ...
		  <div class="card-title text-uppercase text-center py-3">Verify</div>
			...
			<button type="submit" name="change_btn" class="btn btn-primary shadow-primary btn-block waves-effect waves-light">Verify</button>
			</form>
		   </div>
		  </div>
