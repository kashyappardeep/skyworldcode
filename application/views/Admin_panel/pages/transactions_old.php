<?php
$ttl = $this->conn->runQuery('count(id)as total', 'transaction_bkp', "1=1");
$ttl_records = $ttl[0]->total;

?>

<div class="row pt-2 pb-2">
    <div class="col-sm-9">
        <h4 class="page-title"> Transactions Old</h4>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="<?= $admin_path . 'dashboard'; ?>">home</a></li>
            <li class="breadcrumb-item"><a href="#">Transactions Old</a></li>
...
<h6 class="text-uppercase"> Transactions(<?= $ttl_records ?>)</h6>
<hr>
<?php
...
<div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <form action="<?= $admin_path . 'transaction_old'; ?>" method="get">
            <div class="form-inline">
                ...
                <div class="form-group">
                    <select name="select_wallet" class="form-control" id="">
                        <option value="">Select Wallet</option>
                        <option value="main_wallet" <?= isset($_REQUEST['select_wallet']) && $_REQUEST['select_wallet'] == 'main_wallet' ? 'selected' : ''; ?>>Main Wallet</option>
                        <option value="fund_wallet" <?= isset($_REQUEST['select_wallet']) && $_REQUEST['select_wallet'] == 'fund_wallet' ? 'selected' : ''; ?>>Fund Wallet</option>
                    </select>
                </div>
                ...
                <a href="<?= $admin_path . 'transaction_old'; ?>" class="btn btn-sm m-1"> Reset </a>
        ...
        ?>
        <?php
        // $ttl=$this->conn->runQuery('sum(amount)as total,sum(tx_charge)as charge','transaction',"1=1");
        // print_r($ttl_amnt[0]->charge);
        // die();
        $ttl_amnt = $ttl_amnts[0]->total;
        $ttl_tx_charge = $ttl_amnts[0]->charge;
        ?>
        <div align="right">
                ...
                <tbody>
                    ...
                    ?>
                            <tr>
                                ...
                                <td><?php if ($tx_profile_from) {
                                        echo $tx_profile_from->name . '( ' . $tx_profile_from->username . ' )';
                                    } ?></td>
                                <td><?php if ($t_data['wallet_type'] == 'main_wallet') {
                                        echo "Main Wallet";
                                    } elseif ($t_data['wallet_type'] == 'fund_wallet') {
                                        echo "Fund Wallet";
                                    } ?></td>
