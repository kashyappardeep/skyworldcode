...
<div class="row pt-2 pb-2">
        <div class="col-sm-12">
            ...
            <div class="card card-body card-bg-1">
                <form action="<?= $panel_path.'income/level_report';?>" method="get">
                        ...
                        $my_gen=$this->team->my_level_team($u_code);
                          ...
                          if($plan){
                              ...
                              foreach($plan as $plan_details){
                                   ...
                                   $order_details=$this->conn->runQuery('SUM(amount) as amnt','transaction',"u_code='$u_code' and source='level' and tx_record='$level' $whr ")[0]->amnt;
                                   if(array_key_exists($level,$my_gen)){
                                       $ids=$my_gen[$level];
                                       
                                       $business_volume=$this->business->purchase_volume_by_date($ids,$start_date,$end_date);
                                   }else{
                                       $business_volume=0;
                                   }
